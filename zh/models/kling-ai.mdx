...
- **重大变更（2025-12-19）**：新增 `text-to-video` 类型支持（多模型支持）；新增 `sound` 参数（仅 V2.6，开启音频生成）；新增 `aspect_ratio` 参数（仅 V2.6，支持 16:9、9:16、1:1）；新增 `kling-v2-6` 模型版本。
...
| 模型 | text-to-video | image-to-video | sound | aspect_ratio | 说明 |
|------|:-------------:|:--------------:|:-----:|:------------:|------|
| `kling-v1` | ✅ | ✅ | ❌ | ❌ | 基础版本 |
| `kling-v1-5` | ❌ | ✅ | ❌ | ❌ | 迭代版本（仅图生视频） |
| `kling-v1-6` | ✅ | ✅ | ❌ | ❌ | 迭代版本 |
| `kling-v2-master` | ✅ | ✅ | ❌ | ❌ | 高质量版本 |
| `kling-v2-1` | ❌ | ✅ | ❌ | ❌ | 高质量版本（仅图生视频） |
| `kling-v2-1-master` | ✅ | ✅ | ❌ | ❌ | 高质量版本 |
...
| type | 说明 |
|------|------|
| `simple` | 简单运镜，需配合 `config` 参数 |
| `down_back` | 镜头下压并后退 |
| `forward_up` | 镜头前进并上仰 |
| `right_turn_forward` | 右旋后前进 |
| `left_turn_forward` | 左旋后前进 |

### config 参数（仅 simple 类型）

| 参数 | 范围 | 说明 |
|------|------|------|
| `horizontal` | [-10, 10] | 水平运镜 |
| `vertical` | [-10, 10] | 垂直运镜 |
| `pan` | [-10, 10] | 水平摇镜 |
| `tilt` | [-10, 10] | 垂直摇镜 |
| `roll` | [-10, 10] | 旋转运镜 |
...
- **哪些模型支持文生视频？** 支持 `text-to-video` 的模型：`kling-v1`、`kling-v1-6`、`kling-v2-master`、`kling-v2-1-master`、`kling-v2-5-turbo`、`kling-v2-6`。注意 `kling-v1-5` 和 `kling-v2-1` 仅支持图生视频。
...
- **如何使用镜头控制？** 设置 `camera_control`，镜头控制同时支持 text-to-video 和 image-to-video；`simple` 模式的 6 个 `config` 参数只能有一个非零。
