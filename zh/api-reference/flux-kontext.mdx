...
<Warning>
  **Breaking Change（2025-12-10）**：参考图字段已从
  `input_image`/`input_image_2`/`_3`/`_4` 统一为 `image_urls` 数组（最多 4
  张）。
</Warning>
...
官方价格（Replicate）：Pro $0.04/张，Max $0.08/张。我们的价格：Pro 30 积分，Max 60 积分（1000 积分 = $1）。
...
| `image_urls`        | `array<string>` | 参考图 URL 列表            | 最多 4 张，建议 1-3 张；JPEG/PNG/WebP；≤30MB/张 |
...
**多参考图**：使用 `image_urls` 数组，支持最多 4 张参考图，会产生额外费用（Pro +3 积分/张，Max +5 积分/张）。
...
#### Flux Kontext Pro
- 基础：30 积分
- 对比官方（Replicate $0.04）：节省 25%

#### Flux Kontext Max
- 基础：60 积分
- 对比官方（Replicate $0.08）：节省 25%

### 参数速览与规则

- **必填**：`prompt`
- **可选**：`aspect_ratio`、`image_urls`、`output_format`、`prompt_upsampling`、`safety_tolerance`、`seed`、`webhook_url`、`webhook_secret`
- **规则提示**：
  - 宽高比必须在 9:21 到 21:9 之间
  - 参考图使用 `image_urls`，最多 4 张
  - 安全容忍度：0（最严格）到 6（最宽松）
